...
public class UserRestrictedEntityDao {
    ...
    public List<PersistentDashboard> getAllowedDashboards() {
        TypedQuery<PersistentDashboard> query = persistence.getEntityManager()
                .createQuery("select dash from dashboard$PersistentDashboard dash where dash.isAvailableForAllUsers = true or dash.createdBy = :login", PersistentDashboard.class);
    ...
    public int getAllowedDashboardsCount() {
        Query query = persistence.getEntityManager()
                .createQuery("select count(dash.id) from dashboard$PersistentDashboard dash where dash.isAvailableForAllUsers = true or dash.createdBy = :login");
        query.setParameter("login", getCurrentSessionLogin());
    ...
    public List<WidgetTemplate> getAllowedWidgetTemplates() {
        TypedQuery<WidgetTemplate> query = persistence.getEntityManager()
                .createQuery("select wt from dashboard$WidgetTemplate wt where wt.isAvailableForAllUsers = true or wt.createdBy = :login", WidgetTemplate.class);
        query.setParameter("login", getCurrentSessionLogin());
